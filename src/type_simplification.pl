...
tabling_simplify_step2 :-
    ...
    ; get_analysis_types(TypeRules),
      ...
      select_simplify_some_typedefs(TypeRules)
    ).
...
select_simplify_some_typedefs(TypeRules):-
    ...
    simplify_some_typedefs_(NoAutoRules, TypeRules).
...
simplify_some_typedefs(TypeRules):-
    ...
    simplify_some_typedefs_(NoAutoRules, NewSimAutoRules).
...
simplify_some_typedefs_(NoAutoRules, NewSimAutoRules):-
    ...
    replace_all_equiv_types_in_renamings,
    unfold_all_types_in_renamings.
    % replace_all_non_par_types_in_rules(RenRules6, _NewRules).
    %
    % This is not necessary since it is supposed that the non-parametric 
    % which are a renaming of a parametric type rule instance are at the beginning 
    % in the list of non-parametric type rules.
...
partition_par_type_renamings([NonParRule|Rules], [NonParRule|RenamingRules], OtherRules):-
    NonParRule = typedef(NTypeSymbol, _NDefin), non_par_rule_type_symbol(NTypeSymbol),
...
type_rule_list(A) :- list(type_rule, A).
...
:- pred replace_single_types(SingleRules, DisjunctionRules, ReplacedRules, Flag)
   # "Replaces all the types symbols in @var{SingleRules} (which are
      defined by only one disjunct) by their definitions in all the
      rules in @var{DisjunctionRules} obtaining @var{ReplacedRules}
      @var{Flag} is not used currently.".
...
%%    type have been colapsed (the first occurrence is left).
...
:- pred get_required_rules(+TypeSymbolList, -TypeRuleList)
   # "Get all the type rules @var{TypeRuleList} which define the type
      symbols in @var{TypeSymbolList} (and the types appearing in any
      rule of @var{TypeRuleList}, i.e. compute the transitive closure
      of @var{TypeSymbolList}).".
...
compute_transitive_closure([TypeSymbol|TypeSymbolList], Seen, TransClosure):-
    compute_one_type_closure([TypeSymbol|TypeSymbolList], Seen, NewSeen, NewTypSymList),
...
select_rules_3([TypSymbol|List], ReqRules):-
    ( get_typedef(TypSymbol, Def) ->
        Rule1 = typedef(TypSymbol, Def), non_par_rule_type_symbol(TypSymbol),
...
selected_type(TypSymbol):-
    ( internally_defined_type_symbol(TypSymbol,1) ; is_type_param_symbol(TypSymbol) ),
...
:- pred selec_type_rule_simplify
   # "Simplify rules whose types symbols are asserted in a fact
      no_simplified_type(TypSymbol).  First remove empty types, and
      then unfold the remaining rules.".
...
:- pred check_and_remove_empty_types(+TypeRuleList, -OutRules)
   # "Checks which types are empty and remove them from the rule base.
      If a (non-parametric) rule type symbol @verb{T} (defined by a
      type rule) is detected to be empty, then gives a warning message
      and retracts the type defining @verb{T} (a rule type symbol
      without defining rule is considered as empty).  @var{OutRules}
      is @var{TypeRuleList} minus rules defining empty types (this
      rules are retracted as datafacts).".
...
:- pred remove_empty_types(+RuleList)
   # "For each type rule @verb{R} in @var{RuleList} obtains a type
      rule @verb{NR} by removing empty types from @verb{R}. It is
      assumed that @verb{R} is currently asserted. @verb{R} is
      retracted and @verb{NR} asserted.".
...
:- export(replace_non_par_rule_type_symbol/2). % (internal?)
...
% ---------------------------------------------------------------------------
%! # GC roots for types (required types)
