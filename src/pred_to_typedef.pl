...
%! # Predicate to typedef translation
...
:- pred all_occurs_vars(?Term, -VarList)
   # "Creates a (closed) list of variables @var{VarList} with the
      occurrences of variables in the term @var{Term}. Note that
...
clause_to_type(InClause, Type, ParametricVars):-
    ...
    prop_unapply(Head,Head2,Arg1),
    Head2 =.. [_Pred|ParametricVars],
